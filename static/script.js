...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const state = {
    text: '',
    bot: 'xbot', // Reverted to xbot
    speed: 0.2, // Increased from 0.1
    baseSpeed: 0.2, // Increased from 0.1
    speedMultiplier: 1.0,
    pause: 300, // Reduced from 800
    listening: false,
    animations: [],
    scene: null,
    camera: null,
    ...
    animationGeneration: 0 // Track animation generation to invalid old timers
  };
  ...
  function addTextStep(animCommand) {
    if (!state.textTimer) {
      if (processedTextField) {
        processedTextField.value += animCommand[1];
        processedTextField.scrollTop = processedTextField.scrollHeight; // Auto-scroll
      }
      state.textTimer = true;
      const currentGen = state.animationGeneration;

      setTimeout(() => {
        // If generation changed, this timeout is from old animation sequence - ignore it
        if (state.animationGeneration !== currentGen) return;
  ...
  function processAnimation(text) {
    ...
    state.animations = [];
    state.characters = [];
    state.textTimer = false; // Reset timer flag immediately
    state.animationGeneration++; // Invalidate pending timeouts

    if (processedTextField) processedTextField.value = '';
    ...
    // If loop was dead but pending flag stuck, reset it to allow restart
    if (state.pending && !state.animations.length) {
      state.pending = false;
    }
